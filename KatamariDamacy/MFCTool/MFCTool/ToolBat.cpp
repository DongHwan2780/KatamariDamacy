...
CToolBat::CToolBat(DEVICES)
	:CObj(pDevice, pDeviceContext)
{
}

CToolBat::CToolBat(const CToolBat & other)
	: CObj(other)
{
}

HRESULT CToolBat::Initialize_Prototype()
{
...
HRESULT CToolBat::Initialize_Clone(void * pArg)
{
...
_int CToolBat::Update(_double DeltaTime)
{
...
_int CToolBat::Late_Update(_double DeltaTime)
{
...
HRESULT CToolBat::SetUp_Components()
{
	/* For.Com_Renderer */
	...
	/* For.Com_Model */
	if (FAILED(__super::SetUp_Components(STATIC_SCENE, L"Component_Model_Bat", L"Com_Model", (CComponent**)&m_pModel)))
		return E_FAIL;
...
HRESULT CToolBat::SetUp_ConstantTable()
{
	if (nullptr == m_pModel)
		return E_FAIL;
...
CToolBat * CToolBat::Create(DEVICES)
{
	CToolBat*		pInstance = new CToolBat(pDevice, pDeviceContext);
...
CObj * CToolBat::Clone(void * pArg)
{
	CToolBat*		pInstance = new CToolBat(*this);
