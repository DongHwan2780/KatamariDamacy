...
CToolBag::CToolBag(DEVICES)
	:CObj(pDevice, pDeviceContext)
{
}

CToolBag::CToolBag(const CToolBag & other)
	: CObj(other)
{
}

HRESULT CToolBag::Initialize_Prototype()
{
...
HRESULT CToolBag::Initialize_Clone(void * pArg)
{
...
_int CToolBag::Update(_double DeltaTime)
{
...
_int CToolBag::Late_Update(_double DeltaTime)
{
...
HRESULT CToolBag::SetUp_Components()
{
	/* For.Com_Renderer */
	...
	/* For.Com_Model */
	if (FAILED(__super::SetUp_Components(STATIC_SCENE, L"Component_Model_Bag", L"Com_Model", (CComponent**)&m_pModel)))
		return E_FAIL;
...
HRESULT CToolBag::SetUp_ConstantTable()
{
	if (nullptr == m_pModel)
		return E_FAIL;
...
CToolBag * CToolBag::Create(DEVICES)
{
	CToolBag*		pInstance = new CToolBag(pDevice, pDeviceContext);
...
CObj * CToolBag::Clone(void * pArg)
{
	CToolBag*		pInstance = new CToolBag(*this);
