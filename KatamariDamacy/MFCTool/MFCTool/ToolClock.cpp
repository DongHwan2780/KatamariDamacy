...
CToolClock::CToolClock(DEVICES)
	:CObj(pDevice, pDeviceContext)
{
}

CToolClock::CToolClock(const CToolClock & other)
	: CObj(other)
{
}

HRESULT CToolClock::Initialize_Prototype()
{
...
HRESULT CToolClock::Initialize_Clone(void * pArg)
{
...
_int CToolClock::Update(_double DeltaTime)
{
...
_int CToolClock::Late_Update(_double DeltaTime)
{
...
HRESULT CToolClock::SetUp_Components()
{
	/* For.Com_Renderer */
	...
	/* For.Com_Model */
	if (FAILED(__super::SetUp_Components(STATIC_SCENE, L"Component_Model_Clock", L"Com_Model", (CComponent**)&m_pModel)))
		return E_FAIL;
...
HRESULT CToolClock::SetUp_ConstantTable()
{
	if (nullptr == m_pModel)
		return E_FAIL;
...
CToolClock * CToolClock::Create(DEVICES)
{
	CToolClock*		pInstance = new CToolClock(pDevice, pDeviceContext);
...
CObj * CToolClock::Clone(void * pArg)
{
	CToolClock*		pInstance = new CToolClock(*this);
