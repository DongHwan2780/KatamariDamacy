#include "..\Headers\VIBuffer_UI.h"

CVIBuffer_UI::CVIBuffer_UI(ID3D11Device * pDevice, ID3D11DeviceContext * pDeviceContext)
	: CVIBuffer(pDevice, pDeviceContext)
{
}

CVIBuffer_UI::CVIBuffer_UI(const CVIBuffer_UI & other)
	: CVIBuffer(other)
{
}

HRESULT CVIBuffer_UI::Initialize_Prototype(const _tchar * pShaderFilePath)
{
...
HRESULT CVIBuffer_UI::Initialize_Clone(void * pArg)
{
	return S_OK;
}

CVIBuffer_UI * CVIBuffer_UI::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pDeviceContext, const _tchar * pShaderFilePath)
{
	CVIBuffer_UI*	pInstance = new CVIBuffer_UI(pDevice, pDeviceContext);

	if (FAILED(pInstance->Initialize_Prototype(pShaderFilePath)))
	{
		MSG_BOX("Failed To Creating CVIBuffer_UI");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CComponent * CVIBuffer_UI::Clone(void * pArg)
{
	CVIBuffer_UI*	pInstance = new CVIBuffer_UI(*this);
