...
HRESULT CBag::Initialize_Clone(void * pArg)
{
...
_int CBag::Update(_double DeltaTime)
{
...
_int CBag::Late_Update(_double DeltaTime)
{
...
HRESULT CBag::SetUp_Components()
{
	...
	ModelDesc.iModelIndexNum = 1;
	if (FAILED(__super::SetUp_Components(STAGEONE_SCENE, L"Component_Model_Bag", L"Com_Model", (CComponent**)&m_pModel, &ModelDesc)))
		return E_FAIL;
...
CBag * CBag::Create(DEVICES)
{
	CBag*		pInstance = new CBag(pDevice, pDeviceContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX("Failed to Creating CBag");
