...
CEraser::CEraser(DEVICES)
	:CObjList(pDevice, pDeviceContext)
{
}

CEraser::CEraser(const CEraser & other)
	: CObjList(other)
{
}

HRESULT CEraser::Initialize_Prototype()
{
...
HRESULT CEraser::Initialize_Clone(void * pArg)
{
...
_int CEraser::Update(_double DeltaTime)
{
...
_int CEraser::Late_Update(_double DeltaTime)
{
...
HRESULT CEraser::SetUp_Components()
{
	/* For.Com_Renderer */
	...
	ModelDesc.iModelIndexNum = 16;
	if (FAILED(__super::SetUp_Components(STAGEONE_SCENE, L"Component_Model_Eraser", L"Com_Model", (CComponent**)&m_pModel, &ModelDesc)))
		return E_FAIL;
...
HRESULT CEraser::SetUp_ConstantTable()
{
	if (nullptr == m_pModel)
		return E_FAIL;
...
CEraser * CEraser::Create(DEVICES)
{
	CEraser*		pInstance = new CEraser(pDevice, pDeviceContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX("Failed to Creating CEraser");
...
CObj * CEraser::Clone(void * pArg)
{
	CEraser*		pInstance = new CEraser(*this);
