#include "stdafx.h"
#include "..\Headers\Stand.h"

#include "Management.h"


CStand::CStand(DEVICES)
	:CObjList(pDevice, pDeviceContext)
{
}

CStand::CStand(const CStand & other)
	: CObjList(other)
{
}

HRESULT CStand::Initialize_Prototype()
{
...
HRESULT CStand::Initialize_Clone(void * pArg)
{
...
_int CStand::Update(_double DeltaTime)
{
...
_int CStand::Late_Update(_double DeltaTime)
{
...
HRESULT CStand::SetUp_Components()
{
	/* For.Com_Renderer */
	...
	ModelDesc.iModelIndexNum = 29;
	if (FAILED(__super::SetUp_Components(STAGEONE_SCENE, L"Component_Model_Stand", L"Com_Model", (CComponent**)&m_pModel, &ModelDesc)))
		return E_FAIL;
...
HRESULT CStand::SetUp_ConstantTable()
{
	if (nullptr == m_pModel)
		return E_FAIL;
...
CStand * CStand::Create(DEVICES)
{
	CStand*		pInstance = new CStand(pDevice, pDeviceContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX("Failed to Creating CStand");
...
CObj * CStand::Clone(void * pArg)
{
	CStand*		pInstance = new CStand(*this);
