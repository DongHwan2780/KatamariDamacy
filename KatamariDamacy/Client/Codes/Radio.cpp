#include "stdafx.h"
#include "..\Headers\Radio.h"

#include "Management.h"

CRadio::CRadio(DEVICES)
	:CObjList(pDevice, pDeviceContext)
{
}

CRadio::CRadio(const CRadio & other)
	: CObjList(other)
{
}
...
HRESULT CRadio::Initialize_Clone(void * pArg)
{
...
_int CRadio::Update(_double DeltaTime)
{
...
_int CRadio::Late_Update(_double DeltaTime)
{
...
HRESULT CRadio::SetUp_Components()
{
	/* For.Com_Renderer */
	...
	ModelDesc.iModelIndexNum = 27;
	if (FAILED(__super::SetUp_Components(STAGEONE_SCENE, L"Component_Model_Radio", L"Com_Model", (CComponent**)&m_pModel, &ModelDesc)))
		return E_FAIL;
...
HRESULT CRadio::SetUp_ConstantTable()
{
	if (nullptr == m_pModel)
		return E_FAIL;
...
CRadio * CRadio::Create(DEVICES)
{
	CRadio*		pInstance = new CRadio(pDevice, pDeviceContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX("Failed to Creating CRadio");
