...
CCandy::CCandy(DEVICES)
	:CObjList(pDevice, pDeviceContext)
{
}

CCandy::CCandy(const CCandy & other)
	: CObjList(other)
{
}
...
HRESULT CCandy::Initialize_Clone(void * pArg)
{
...
_int CCandy::Update(_double DeltaTime)
{
...
_int CCandy::Late_Update(_double DeltaTime)
{
...
HRESULT CCandy::SetUp_Components()
{
	/* For.Com_Renderer */
	...
	ModelDesc.iModelIndexNum = 10;
	if (FAILED(__super::SetUp_Components(STAGEONE_SCENE, L"Component_Model_Candy", L"Com_Model", (CComponent**)&m_pModel, &ModelDesc)))
		return E_FAIL;
...
HRESULT CCandy::SetUp_ConstantTable()
{
	if (nullptr == m_pModel)
		return E_FAIL;
...
CCandy * CCandy::Create(DEVICES)
{
	CCandy*		pInstance = new CCandy(pDevice, pDeviceContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX("Failed to Creating CCandy");
...
CObj * CCandy::Clone(void * pArg)
{
	CCandy*		pInstance = new CCandy(*this);
