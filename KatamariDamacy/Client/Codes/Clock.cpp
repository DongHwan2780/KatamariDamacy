...
HRESULT CClock::Initialize_Clone(void * pArg)
{
...
_int CClock::Update(_double DeltaTime)
{
...
_int CClock::Late_Update(_double DeltaTime)
{
...
HRESULT CClock::SetUp_Components()
{
	/* For.Com_Renderer */
	...
	ModelDesc.iModelIndexNum = 11;
	if (FAILED(__super::SetUp_Components(STAGEONE_SCENE, L"Component_Model_Clock", L"Com_Model", (CComponent**)&m_pModel, &ModelDesc)))
		return E_FAIL;
...
HRESULT CClock::SetUp_ConstantTable()
{
	if (nullptr == m_pModel)
		return E_FAIL;
...
CClock * CClock::Create(DEVICES)
{
	CClock*		pInstance = new CClock(pDevice, pDeviceContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX("Failed to Creating CClock");
...
CObj * CClock::Clone(void * pArg)
{
	CClock*		pInstance = new CClock(*this);
