...
HRESULT CEgg::Initialize_Prototype()
{
...
HRESULT CEgg::Initialize_Clone(void * pArg)
{
...
_int CEgg::Update(_double DeltaTime)
{
...
_int CEgg::Late_Update(_double DeltaTime)
{
...
HRESULT CEgg::SetUp_Components()
{
	...
	ModelDesc.iModelIndexNum = 15;
	if (FAILED(__super::SetUp_Components(STAGEONE_SCENE, L"Component_Model_Egg", L"Com_Model", (CComponent**)&m_pModel, &ModelDesc)))
		return E_FAIL;
...
HRESULT CEgg::SetUp_ConstantTable()
{
	if (nullptr == m_pModel)
		return E_FAIL;
...
CEgg * CEgg::Create(DEVICES)
{
	CEgg*		pInstance = new CEgg(pDevice, pDeviceContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX("Failed to Creating CEgg");
...
CObj * CEgg::Clone(void * pArg)
{
	CEgg*		pInstance = new CEgg(*this);
