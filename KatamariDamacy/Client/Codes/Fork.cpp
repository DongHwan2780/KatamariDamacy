...
HRESULT CFork::Initialize_Clone(void * pArg)
{
...
_int CFork::Update(_double DeltaTime)
{
...
_int CFork::Late_Update(_double DeltaTime)
{
...
HRESULT CFork::SetUp_Components()
{
	...
	ModelDesc.iModelIndexNum = 17;
	if (FAILED(__super::SetUp_Components(STAGEONE_SCENE, L"Component_Model_Fork", L"Com_Model", (CComponent**)&m_pModel, &ModelDesc)))
		return E_FAIL;
...
CFork * CFork::Create(DEVICES)
{
	CFork*		pInstance = new CFork(pDevice, pDeviceContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX("Failed to Creating CFork");
