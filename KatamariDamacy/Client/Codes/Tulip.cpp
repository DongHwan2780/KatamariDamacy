#include "stdafx.h"
#include "..\Headers\Tulip.h"

#include "Management.h"

CTulip::CTulip(DEVICES)
	:CObjList(pDevice, pDeviceContext)
{
}

CTulip::CTulip(const CTulip & other)
	: CObjList(other)
{
}

HRESULT CTulip::Initialize_Prototype()
{
...
HRESULT CTulip::Initialize_Clone(void * pArg)
{
...
_int CTulip::Update(_double DeltaTime)
{
...
_int CTulip::Late_Update(_double DeltaTime)
{
...
HRESULT CTulip::SetUp_Components()
{
	/* For.Com_Renderer */
...
HRESULT CTulip::SetUp_ConstantTable()
{
	if (nullptr == m_pModel)
		return E_FAIL;
...
CTulip * CTulip::Create(DEVICES)
{
	CTulip*		pInstance = new CTulip(pDevice, pDeviceContext);
...
CObj * CTulip::Clone(void * pArg)
{
	CTulip*		pInstance = new CTulip(*this);
