#include "stdafx.h"
#include "..\Headers\Bansai.h"

#include "Management.h"

CBansai::CBansai(DEVICES)
	:CObjList(pDevice, pDeviceContext)
{
}

CBansai::CBansai(const CBansai & other)
	: CObjList(other)
{
}

HRESULT CBansai::Initialize_Prototype()
{
...
HRESULT CBansai::Initialize_Clone(void * pArg)
{
...
_int CBansai::Update(_double DeltaTime)
{
...
_int CBansai::Late_Update(_double DeltaTime)
{
...
HRESULT CBansai::SetUp_Components()
{
	/* For.Com_Renderer */
	...
	ModelDesc.iModelIndexNum = 4;
	if (FAILED(__super::SetUp_Components(STAGEONE_SCENE, L"Component_Model_Bansai", L"Com_Model", (CComponent**)&m_pModel, &ModelDesc)))
		return E_FAIL;
...
HRESULT CBansai::SetUp_ConstantTable()
{
	if (nullptr == m_pModel)
		return E_FAIL;
...
CBansai * CBansai::Create(DEVICES)
{
	CBansai*		pInstance = new CBansai(pDevice, pDeviceContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX("Failed to Creating CBansai");
...
CObj * CBansai::Clone(void * pArg)
{
	CBansai*		pInstance = new CBansai(*this);
