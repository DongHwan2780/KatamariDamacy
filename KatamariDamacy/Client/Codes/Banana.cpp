...
CBanana::CBanana(DEVICES)
	:CObjList(pDevice, pDeviceContext)
{
}

CBanana::CBanana(const CBanana & other)
	: CObjList(other)
{
}

HRESULT CBanana::Initialize_Prototype()
{
...
HRESULT CBanana::Initialize_Clone(void * pArg)
{
...
_int CBanana::Update(_double DeltaTime)
{
...
_int CBanana::Late_Update(_double DeltaTime)
{
...
HRESULT CBanana::SetUp_Components()
{
	/* For.Com_Renderer */
	...
	ModelDesc.iModelIndexNum = 2;
	if (FAILED(__super::SetUp_Components(STAGEONE_SCENE, L"Component_Model_Banana", L"Com_Model", (CComponent**)&m_pModel, &ModelDesc)))
		return E_FAIL;
...
HRESULT CBanana::SetUp_ConstantTable()
{
	if (nullptr == m_pModel)
		return E_FAIL;
...
CBanana * CBanana::Create(DEVICES)
{
	CBanana*		pInstance = new CBanana(pDevice, pDeviceContext);
...
CObj * CBanana::Clone(void * pArg)
{
	CBanana*		pInstance = new CBanana(*this);
