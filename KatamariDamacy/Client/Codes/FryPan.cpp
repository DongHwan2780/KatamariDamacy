#include "stdafx.h"
#include "..\Headers\FryPan.h"

#include "Management.h"

CFryPan::CFryPan(DEVICES)
	:CObjList(pDevice, pDeviceContext)
{
}

CFryPan::CFryPan(const CFryPan & other)
	: CObjList(other)
{
}

HRESULT CFryPan::Initialize_Prototype()
{
...
HRESULT CFryPan::Initialize_Clone(void * pArg)
{
...
_int CFryPan::Update(_double DeltaTime)
{
...
_int CFryPan::Late_Update(_double DeltaTime)
{
...
HRESULT CFryPan::SetUp_Components()
{
	/* For.Com_Renderer */
	...
	ModelDesc.iModelIndexNum = 18;
	if (FAILED(__super::SetUp_Components(STAGEONE_SCENE, L"Component_Model_FryPan", L"Com_Model", (CComponent**)&m_pModel, &ModelDesc)))
		return E_FAIL;
...
HRESULT CFryPan::SetUp_ConstantTable()
{
	if (nullptr == m_pModel)
		return E_FAIL;
...
CFryPan * CFryPan::Create(DEVICES)
{
	CFryPan*		pInstance = new CFryPan(pDevice, pDeviceContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX("Failed to Creating CFryPan");
...
CObj * CFryPan::Clone(void * pArg)
{
	CFryPan*		pInstance = new CFryPan(*this);
