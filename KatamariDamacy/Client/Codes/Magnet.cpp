#include "stdafx.h"
#include "..\Headers\Magnet.h"

#include "Management.h"

CMagnet::CMagnet(DEVICES)
	:CObjList(pDevice, pDeviceContext)
{
}

CMagnet::CMagnet(const CMagnet & other)
	: CObjList(other)
{
}
...
HRESULT CMagnet::Initialize_Clone(void * pArg)
{
...
_int CMagnet::Update(_double DeltaTime)
{
...
_int CMagnet::Late_Update(_double DeltaTime)
{
...
HRESULT CMagnet::SetUp_Components()
{
	/* For.Com_Renderer */
	...
	ModelDesc.iModelIndexNum = 20;
	if (FAILED(__super::SetUp_Components(STAGEONE_SCENE, L"Component_Model_Magnet", L"Com_Model", (CComponent**)&m_pModel, &ModelDesc)))
		return E_FAIL;
...
HRESULT CMagnet::SetUp_ConstantTable()
{
	if (nullptr == m_pModel)
		return E_FAIL;
...
CMagnet * CMagnet::Create(DEVICES)
{
	CMagnet*		pInstance = new CMagnet(pDevice, pDeviceContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX("Failed to Creating CMagnet");
