...
CDice::CDice(const CDice & other)
	: CObjList(other)
{
}

HRESULT CDice::Initialize_Prototype()
{
...
HRESULT CDice::Initialize_Clone(void * pArg)
{
...
_int CDice::Update(_double DeltaTime)
{
...
_int CDice::Late_Update(_double DeltaTime)
{
...
HRESULT CDice::SetUp_Components()
{
	/* For.Com_Renderer */
	...
	ModelDesc.iModelIndexNum = 12;
	if (FAILED(__super::SetUp_Components(STAGEONE_SCENE, L"Component_Model_Dice", L"Com_Model", (CComponent**)&m_pModel, &ModelDesc)))
		return E_FAIL;
...
CDice * CDice::Create(DEVICES)
{
	CDice*		pInstance = new CDice(pDevice, pDeviceContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX("Failed to Creating CDice");
