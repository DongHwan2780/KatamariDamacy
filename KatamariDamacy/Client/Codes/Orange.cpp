#include "stdafx.h"
#include "..\Headers\Orange.h"

#include "Management.h"

COrange::COrange(DEVICES)
	:CObjList(pDevice, pDeviceContext)
{
}

COrange::COrange(const COrange & other)
	: CObjList(other)
{
}

HRESULT COrange::Initialize_Prototype()
{
...
HRESULT COrange::Initialize_Clone(void * pArg)
{
...
_int COrange::Update(_double DeltaTime)
{
...
_int COrange::Late_Update(_double DeltaTime)
{
...
HRESULT COrange::SetUp_Components()
{
	/* For.Com_Renderer */
	...
	ModelDesc.iModelIndexNum = 23;
	if (FAILED(__super::SetUp_Components(STAGEONE_SCENE, L"Component_Model_Orange", L"Com_Model", (CComponent**)&m_pModel, &ModelDesc)))
		return E_FAIL;
...
HRESULT COrange::SetUp_ConstantTable()
{
	if (nullptr == m_pModel)
		return E_FAIL;
...
COrange * COrange::Create(DEVICES)
{
	COrange*		pInstance = new COrange(pDevice, pDeviceContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX("Failed to Creating COrange");
...
CObj * COrange::Clone(void * pArg)
{
	COrange*		pInstance = new COrange(*this);
